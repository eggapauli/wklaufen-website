...
let login (username, password) =
    ...
    |> Async.bind (Choice.bindAsync (fun response -> async {
        let! loginPageContent = response.Content.ReadAsStringAsync() |> Async.AwaitTask
...
module Members =
    let loadAndResetMemberOverviewPage (startPage: HtmlDocument) =
        ...
        |> Async.bind (
            Choice.bindAsync (fun memberPage ->
                ...
                Http.postForm uri formParams
                |> Async.map (Choice.mapError (fun _ -> sprintf "Error while resetting member page"))
            )
        )
        |> Async.bind (Choice.mapAsync loadHtmlResponse)
                    ...
                    Member =
                        {
                            ...
                            Phone = [ getTextBoxValue "mit_mobil"; getTextBoxValue "mit_telefon1"; getTextBoxValue "mit_telefon2" ] |> Parse.firstNonEmptyOrNone
